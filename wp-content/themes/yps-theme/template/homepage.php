<?php
/**
 *  Template Name: Home Page
 *
 * @link https://developer.wordpress.org/themes/basics/template-hierarchy/
 *
 * @package WordPress
  * @subpackage yp-solution
 * @since yp-solution 1.0
 */

get_header();
?>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick.min.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick-theme.min.css">

<main class="cd-main-content">


	<!-- HOME SECTION
		================================================== -->
		<section class="home" style="background: #000">

			<!-- <a href="#scroll-link" class="arrow-down scroll">&#xf103;</a> -->
			<div class="banner-text">
				<h1><?php the_field('banner_text_line_1'); ?></h1>
				<p><?php the_field('subtitle_line1'); ?></p>
			</div>
			<div class="banner_image">
				<?php
				$image = get_field('banner_image');
				if( !empty( $image ) ): ?>
					<img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
				<?php endif; ?>
			</div>



			<div id="particles-js"><!-- <canvas class="particles-js-canvas-el" width="100%" height="852"></canvas> --></div>
			<div class="fottoer_last">
				<div class="fottoer_last_1">
					<a href="#scroll-link" class="scroll">Selected Work</a>
				</div>
				<div class="fottoer_last_2">
					<div class="one_colun">
						<?php
						$image = get_field('banner_fotter_image_1');
						if( !empty( $image ) ): ?>
							<img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
						<?php endif; ?>
					</div>
					<div class="one_colun">
						<?php
						$image = get_field('banner_fotter_image_2');
						if( !empty( $image ) ): ?>
							<img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
						<?php endif; ?>
					</div>
					<div class="one_colun last_colun">
						<ul class="sl-nav-menu">
							<li class=""><a href="#" class="sl-menu-item">IG</a></li>
							<li class=""><a href="#" class="sl-menu-item">TW</a></li>
							<li class=""><a href="#" class="sl-menu-item">FB</a></li>
							<li class=""><a href="#" class="sl-menu-item">LN</a></li>
						</ul>
					</div>
				</div>
			</div>

		</section>


	<!-- SECTION
		================================================== -->
		<section class="section" id="scroll-link">
			<nav class="menu ylp_work">
				<?php
				$args = array(
					'post_type' => 'case_study',
					'cat' => 7,
					'posts_per_page' => 6,
					'orderby' => 'title',
					'order' => 'ASC',
				);

				$loop = new WP_Query( $args ); ?>


				<?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
				<?php $hoverColor = get_field('hover_color') ?>
					<a class="menu__item"
					<?php if( !empty($hoverColor)) { ?> style="backgrond-color:<?php echo $hoverColor ?> " <?php } ?>
					>
						<?php
						$hero = get_field('section_1_client_information');
						if( !empty( $hero ) ): ?>
							<span class="menu__item-text"><span class="menu__item-textinner"><?php print the_title(); ?></span></span>
							<span class="menu__item-sub"><?php print( $hero['services'] ); ?></span>
							<div class="hover-reveal">
								<div class="hover-reveal__img" style="background-image: url(<?php echo esc_url( $hero['home_page_hover_image']['url'] ); ?>);"></div>
							</div>
						<?php endif; ?>
					</a>
				<?php endwhile; ?>

				<?php wp_reset_postdata();  ?>





				<!-- <a class="menu__item" data-img="/maninder/yp-solution/wp-content/themes/yps-theme/img/img/2.jpg">
					<span class="menu__item-text"><span class="menu__item-textinner">Ardor Eyewear</span></span>
					<span class="menu__item-sub">eCommerce / Shopify<br>UX/UI / SEO / Marketing</span>
					<div class="hover-reveal">
						<div class="hover-reveal__img" style="background-image: url(<?php bloginfo('template_url'); ?>/images/cam.jpg);"></div>
					</div>
				</a>
				<a class="menu__item" data-img="/maninder/yp-solution/wp-content/themes/yps-theme/img/img/3.jpg">
					<span class="menu__item-text"><span class="menu__item-textinner">Halo</span></span>
					<span class="menu__item-sub">eCommerce / Shopify<br>UX/UI / SEO / Marketing</span>
					<div class="hover-reveal">
						<div class="hover-reveal__img" style="background-image: url(<?php bloginfo('template_url'); ?>/images/cam.jpg);"></div>
					</div>
				</a>
				<a class="menu__item" data-img="/maninder/yp-solution/wp-content/themes/yps-theme/img/img/4.jpg">
					<span class="menu__item-text"><span class="menu__item-textinner">Cucumber Clothing</span></span>
					<span class="menu__item-sub">eCommerce / Shopify<br>UX/UI / SEO / Marketing</span>
					<div class="hover-reveal">
						<div class="hover-reveal__img" style="background-image: url(<?php bloginfo('template_url'); ?>/images/cam.jpg);"></div>
					</div>
				</a>
				<a class="menu__item" data-img="/maninder/yp-solution/wp-content/themes/yps-theme/img/img/5.jpg">
					<span class="menu__item-text"><span class="menu__item-textinner">Direct Specs</span></span>
					<span class="menu__item-sub">eCommerce / Shopify<br>UX/UI / SEO / Marketing</span>
					<div class="hover-reveal">
						<div class="hover-reveal__img" style="background-image: url(<?php bloginfo('template_url'); ?>/images/cam.jpg);"></div>
					</div>
				</a> -->
			</nav>
		</section>

	<!-- SECTION
		================================================== -->
		<section class="section g_y_section">
			<div class="office-1 mobile_pad">
				<div class="box-1">
					<div class="box-1_inner" data-scroll-reveal="enter left move 200px over 1s after 0.3s">
						<div class="box-1_inner_sub">
							<?php
							$image = get_field('section_3_image');
							if( !empty( $image ) ): ?>
								<img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
							<?php endif; ?>
						</div>
					</div>
					<!-- <ul id="owl-office" class="owl-carousel owl-theme">
						<li><img  src="<?php bloginfo('template_url'); ?>/images/Repeat-Grid-1.jpg" alt="" /></li>
						<li><img  src="<?php bloginfo('template_url'); ?>/images/office2.jpg" alt="" /></li>
						<li><img  src="<?php bloginfo('template_url'); ?>/images/office3.jpg" alt="" /></li>
					</ul> -->
				</div>
				<div class="box-1">
					<div class="text-in">
						<div class="section-title left office-text">
							<svg width="30" height="30" class="squareanimation">
								<rect x="0" y="0" width="30" height="30" style="fill:#F9CF01;" />
							</svg>
							<h2><?php the_field('section_3_title'); ?></h2>
							<div class="subtitle left black_cl"><?php the_field('section_3_subtext'); ?></div>
							<a href="#" class="subtitle-button" role="button">
								<span class="button-text">More about us </span>
							</a>
						</div>
					</div>
				</div>
			</div>
		</section>


	<!-- SECTION
		================================================== -->
		<section class="section black_bg">
			<div class="office-1">
				<div class="box-1">
					<div class="text-in new_wt">
						<div class="section-title left office-text">
							<h3 class="white_cl"><?php the_field('section_4_title'); ?></h3>
							<svg width="30" height="30" class="squareanimation">
								<rect x="0" y="0" width="30" height="30" style="fill:#F9CF01;" />
							</svg>
							<!-- <a href="#" class="subtitle-button" role="button">
								<span class="button-text">More about us </span>
							</a> -->
						</div>
					</div>
				</div>
				<div class="box-1"  data-scroll-reveal="enter right move 200px over 1s after 0.3s">
					<div class="box-1_inner2">
						<div class="box-1_inner_sub2">
							<?php
							$image = get_field('section_4_image');
							if( !empty( $image ) ): ?>
								<img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
							<?php endif; ?>
						</div>
					</div>
				</div>
			</div>
		</section>

	<!-- SECTION
		================================================== -->
		<section class="section black_bg">
			<div class="office-1">
				<div class="our_solution">
					<div class="our_solution_inner">
						<div class="our_solution_head">
							<svg class="spinround" version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 49" width="50" height="49">
								<title>Screenshot_5</title>
								<defs>
									<image  width="50" height="53" id="img1" href="data:image/png;base64,"/>
								</defs>
								<style>
								</style>
								<use id="Background" href="#img1" x="0" y="0" />
							</svg>
							<h2><?php the_field('section_5_title'); ?></h2>
							<div class="divider"></div>
						</div>
						<div class="our_solution_base">
							<div class="our_solution_base1">
								<div class="subtitle left"><?php the_field('section_5_subtile'); ?></div>
								<a href="#" class="subtitle-button white_cl" role="button">
									<span class="button-text">View All </span>
									<li class="blur-container ready">
										<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 556 556" class="svg-blur">
											<defs>
												<filter id="a" width="114.5%" height="114.5%" x="-7.3%" y="-7.3%" filterUnits="objectBoundingBox">
													<feGaussianBlur in="SourceGraphic" stdDeviation="12"></feGaussianBlur>
												</filter>
											</defs>
											<circle cx="1496" cy="1585" r="248" fill="#FFFFFF" fill-rule="evenodd" filter="url(#a)" opacity=".6" transform="translate(-1218 -1307)"></circle>
										</svg>
									</li>
								</a>
							</div>
							<div class="our_solution_base2">
								<div class="our_solution_base2_inn" data-scroll-reveal="enter right move 200px over 1s after 0.2s">
									<div class="our_base_inner">
										<h4 class=""><span class="list_style_num">01</span></h4>
									</div>
									<div class="our_base_inner">
										<h4 class=""><?php the_field('section_5_solution_title1'); ?></h4>
										<ul class="our_base_inner_list">
											<li>
												<!-- 												<a href="#"><span><?php the_field('solution_1_subtitle_1'); ?></span></a> -->

												<?php
												$ecommerce = get_field('solution_1_subtitle_1');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>Ecommerce Strategy</span></a>
												<?php endif; ?>
											</li>
											<li>
												<?php
												$ecommerce = get_field('solution_1_subtitle_2');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>User Experience Design</span></a>
												<?php endif; ?>
											</li>
											<li>
												<?php
												$ecommerce = get_field('solution_1_subtitle_3');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>Conversion Rate Optimisation</span></a>
												<?php endif; ?>
											</li>
										</ul>
									</div>
								</div>
								<div class="divider"></div>
								<div class="our_solution_base2_inn"  data-scroll-reveal="enter right move 200px over 1s after 0.5s">
									<div class="our_base_inner">
										<h4 class=""><span class="list_style_num">02</span></h4>
									</div>
									<div class="our_base_inner">
										<h4 class=""> <?php the_field('section_5_solution_title2'); ?></h4>
										<ul class="our_base_inner_list">
											<li>
												<?php
												$ecommerce = get_field('solution_2_subtitle_1');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>Logo and Branding</span></a>
												<?php endif; ?>
											</li>
											<li>
												<?php
												$ecommerce = get_field('solution_2_subtitle_2');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>Graphics Design</span></a>
												<?php endif; ?>
											</li>
											<li>
												<?php
												$ecommerce = get_field('solution_2_subtitle_3');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>Website Design</span></a>
												<?php endif; ?>
											</li>
										</ul>
									</div>
								</div>
								<div class="divider"></div>
								<div class="our_solution_base2_inn"  data-scroll-reveal="enter right move 200px over 1s after 0.9s">
									<div class="our_base_inner">
										<h4 class=""><span class="list_style_num">03</span></h4>
									</div>
									<div class="our_base_inner">
										<h4 class=""> <?php the_field('section_5_solution_title3'); ?></h4>
										<ul class="our_base_inner_list">
											<li>
												<?php
												$ecommerce = get_field('solution_3_subtitle_1');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>Ecommerce</span></a>
												<?php endif; ?>

											</li>
											<li>
												<?php
												$ecommerce = get_field('solution_3_subtitle_2');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>B2B Commerce</span></a>
												<?php endif; ?>

											</li>
											<li>
												<?php
												$ecommerce = get_field('solution_3_subtitle_3');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>Ecommerce for Opticians</span></a>
												<?php endif; ?>

											</li>
											<li>
												<?php
												$ecommerce = get_field('solution_3_subtitle_4');
												if( !empty( $ecommerce ) ): ?>
													<a href="<?php echo esc_url($ecommerce['url']); ?>"><span>Application Development</span></a>
												<?php endif; ?>

											</li>
										</ul>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</section>

	<!-- SECTION
		================================================== -->
		<section class="section black_bg">
			<div class="office-1">
				<div class="box-1 new_2">
					<div class="text-in">
						<div class="section-title left office-text">
							<h2 class="white_cl"><?php the_field('section_6_title'); ?></h2>
							<div class="subtitle left white_cl"><?php the_field('section_6_subtile1'); ?></div>
							<a href="#" class="subtitle-button" role="button">
								<span class="button-text">More about us </span>
							</a>
						</div>
					</div>
				</div>
				<div class="box-1 new_3"  data-scroll-reveal="enter right move 200px over 1s after 0.3s">
					<div class="box-1_inner2">
						<div class="box-1_inner_sub2">
							<?php
							$image = get_field('section_6_image');
							if( !empty( $image ) ): ?>
								<img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
							<?php endif; ?>
						</div>
					</div>
				</div>
				<div class="box-4">
					<div class="subtitle left white_cl"><?php the_field('section_6_subtile2'); ?></div>
					<a href="<?php the_field('section_6_button'); ?>" class="subtitle-button2" role="button">
						<span class="button-text">Get In Touch</span>
					</a>
					<li class="blur-container ready">
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 556 556" class="svg-blur">
							<defs>
								<filter id="a" width="114.5%" height="114.5%" x="-7.3%" y="-7.3%" filterUnits="objectBoundingBox">
									<feGaussianBlur in="SourceGraphic" stdDeviation="12"></feGaussianBlur>
								</filter>
							</defs>
							<circle cx="1496" cy="1585" r="248" fill="#FFFFFF" fill-rule="evenodd" filter="url(#a)" opacity=".6" transform="translate(-1218 -1307)"></circle>
						</svg>
					</li>
				</div>
			</div>
		</section>

	<!-- SECTION
		================================================== -->
		<section class="section black_bg">
			<div class="platfom_inner">
				<div class="our_solution_head">
					<h2><?php the_field('section_7_title'); ?></h2>
					<div class="divider"></div>
					<div class="subtitle left pas_new"><?php the_field('section_7_subtile'); ?></div>
				</div>
				<div class="platfom_inner_cont">
					<div class="one-third column" data-scroll-reveal="enter left move 200px over 1s after 0.3s">
						<div class="services-boxes-2 grey-section">
							<div class="icon-box">
								<?php
								$image = get_field('section_7_post1_Image');
								if( !empty( $image ) ): ?>
									<img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
								<?php endif; ?>
							</div>
							<p><?php the_field('section_7_post1_content'); ?></p>
						</div>
					</div>
					<div class="one-third column" data-scroll-reveal="enter bottom move 200px over 1s after 0.3s">
						<div class="services-boxes-2 grey-section">
							<div class="icon-box">
								<?php
								$image = get_field('section_7_post2_image_');
								if( !empty( $image ) ): ?>
									<img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
								<?php endif; ?>
							</div>
							<p><?php the_field('section_7_post2_content'); ?></p>
						</div>
					</div>
					<div class="one-third column" data-scroll-reveal="enter right move 200px over 1s after 0.3s">
						<div class="services-boxes-2 grey-section">
							<div class="icon-box">
								<?php
								$image = get_field('section_7_post3_image');
								if( !empty( $image ) ): ?>
									<img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" />
								<?php endif; ?>
							</div>
							<p><?php the_field('section_7_post3_content_'); ?></p>
						</div>
					</div>
				</div>
			</div>
		</section>

	<!-- SECTION
		================================================== -->
		<section class="section parallax-section parallax-section-padding-top-bottom">

			<div class="parallax-6"></div>

			<div class="testimonal_home">
				<div class="sixteen columns">
					<div class="section-title">
						<h2>What People Say About Us</h2>
					</div>
				</div>
				<div class="sixteen columns">
					<div id="owl-blockquotes" class="owl-carousel owl-theme">
						<?php
						$args = array(
							'post_type' => 'testimonial',
							'posts_per_page' => 20,
							'orderby' => 'title',
							'order' => 'DSC',
						);

						$loop = new WP_Query( $args ); ?>

						<?php while ( $loop->have_posts() ) : $loop->the_post(); ?>

							<div class="item blockquotes-1">
								<div class="item-wrap">
									<div class="client-name">
										<h1><?php print the_title(); ?></h1>
										<img src="<?php bloginfo('template_url'); ?>/images/Group-38.png">
									</div>
									<div class="client-work">
										<h5><?php the_field('project_name'); ?></h5>
									</div>
									<div class="client-description">
										<p><?php the_field('project_description'); ?></p>
									</div>
								</div>
							</div>

						<?php endwhile; ?>
						<?php wp_reset_postdata();  ?>
					</div>
				</div>
			</div>
		</section>


	<!-- SECTION
		================================================== -->
		<section class="section logos-slider_section">
			<div class="yp-container">
				<div class="logo-slider-left-move">
                    <?php
                        $args = array(
                            'post_type' => 'home_logo',
                            'cat' => 9,
                            'posts_per_page' => 8,
                            'orderby' => 'title',
                            'order' => 'DSC',
                        );
                        $loop = new WP_Query( $args );
                    ?>
                    <?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
                        <?php $image = get_field('logo'); ?>
                        <?php if( !empty( $image ) ): ?>
                            <div>
                                <a class="logo-bx" href="javascript:void(0);">
                                    <img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" loading="lazy" />
                                </a>
                            </div>
                        <?php endif; ?>
                    <?php endwhile; ?>

                    <?php wp_reset_postdata();  ?>
				</div>

				<div class="logo-slider-right-move" dir="rtl">
                      <?php
                      $args = array(
                          'post_type' => 'home_logo',
                          'cat' => 8,
                          'posts_per_page' => 8,
                          'orderby' => 'title',
                          'order' => 'DSC',
                      );
                      $loop = new WP_Query( $args );
                    ?>
                    <?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
                        <?php $image = get_field('logo'); ?>
                        <?php if( !empty( $image ) ): ?>
                            <div>
                                <a class="logo-bx" href="javascript:void(0);">
                                    <img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" loading="lazy" />
                                </a>
                            </div>
                        <?php endif; ?>
                    <?php endwhile; ?>
                    <?php wp_reset_postdata();  ?>
				</div>
			</div><!-- yp-container -->
		</section><!-- logos-slider_section -->

	<!-- SECTION
		================================================== -->
		<section class="section g_y_section">
			<div class="blog_inner">
				<div class="one-second column" data-scroll-reveal="enter left move 200px over 1s after 0.3s">
					<a href="" class="animsition-link">
						<div class="blog-box-1 grey-section">
							<img src="<?php bloginfo('template_url'); ?>/images/balazs-ketyi-LPWl2pEVGKc-unsplash.jpg" alt="">
								<!-- <div class="blog-date-1">21.11.'14</div>
									<div class="blog-comm-1">3 <span></span></div> -->
									<h6>Latest post</h6>
									<p>Lorem ipsum dolor sit consectetur amet, adipisicing elit.</p>
									<div class="link"></div>
								</div>
							</a>
						</div>
						<div class="one-second column" data-scroll-reveal="enter right move 200px over 1s after 0.3s">
							<a href="" class="animsition-link">
								<div class="blog-box-1 grey-section">
									<img src="<?php bloginfo('template_url'); ?>/images/brooke-cagle-WHWYBmtn3_0-unsplash.jpg" alt="">
								<!-- <div class="blog-date-1">21.11.'14</div>
									<div class="blog-comm-1">3 <span></span></div> -->
									<h6>Latest post</h6>
									<p>Lorem ipsum dolor sit consectetur amet, adipisicing elit.</p>
									<div class="link"></div>
								</div>
							</a>
						</div>
					</div>
				</section>

	<!-- SECTION
		================================================== -->
		<section class="section black_bg">
			<div class="fotter_inner">
				<div class="box-1" data-scroll-reveal="enter left move 200px over 1s after 0.3s">
					<div class="text-in">
						<div class="section-title left office-text">
							<h2 class="white_cl"><?php the_field('home_fotter_title'); ?></h2>
							<div class="subtitle left white_cl"><?php the_field('home_fotter_subtitle'); ?></div>
							<a href="<?php the_field('home_fotter_email'); ?>" class="subtitle-button" role="button">
								<span class="button-text white_cl">hello@weareyellowpixel.com </span>
							</a><a class="subtitle-button" style="padding: 0 12px;"><span class="white_cl">|</span></a>
							<a href="<?php the_field('home_fotter_phone'); ?>" class="subtitle-button " role="button">
								<span class="button-text white_cl"> 0118 402 8938</span>
							</a>
						</div>
					</div>
				</div>
				<div class="box-2"  data-scroll-reveal="enter right move 200px over 1s after 0.3s">
					<div class="box-1_inner2">
					</div>
				</div>
			</div>
		</section>


		<?php
		get_footer();
